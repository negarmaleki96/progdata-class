...
```{r}
gapminder |>
  filter(continent=='Asia', pop>100e+06)
```
...
```{r}
gapminder |>
  filter((country=='China' | country=='Brazil'), pop>100e+06)

gapminder |>
  filter(country %in% c('China', 'Brazil'), pop>100e+06)
```
...
gapminder |>
  mutate(
    ...
    gdpBill=round(grossGDP/10e+08,2)
  )
```
